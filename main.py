...
while running:
    ...
    for projectile, enemy_list in collisions.items():
        for enemy in enemy_list:
            # Réduction de la santé de l'ennemi, augmentation du score, etc.
            enemy.health -= projectile.damage
            player.score += enemy.points

    # Gestion des collisions avec les "bumper"
    for projectile, bumper_list in collisions_with_bumpers.items():
        for bumper in bumper_list:
            # Réduction de la santé du bumper, augmentation du score, etc.
            bumper.health -= projectile.damage
            player.score += bumper.points

    for player, bumper_list in player_collisions_with_bumpers.items():
        for bumper in bumper_list:
            # Réduction de la santé du joueur et du bumper, augmentation du score, etc.
    ...
    for player, enemy_list in player_collisions.items():
        for enemy in enemy_list:
            # Réduction de la santé du joueur et de l'ennemi, augmentation du score, etc.
    ...
    bumper_loop += dt
    if bumper_loop >= 250:  # Vous pouvez ajuster cette condition selon vos besoins
        game.spawn_bumper(all_sprites, bumpers, bumper_anim, velocity)
        bumper_loop = 0
        bumper_loop += dt
